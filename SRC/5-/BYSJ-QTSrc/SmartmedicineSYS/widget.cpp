    ...
    ui(new Ui::Widget)
{
    ...
    this->setWindowTitle(QString::fromLocal8Bit("智能药柜"));
    ...
    connect(setDialog,SIGNAL(wifiInitsender()),this,SLOT(InitWifiDev()));
...
void Widget::InitWifiDev(void)
{
    ...
    if(wifithread->SocketInit())
    {
        ...
        connect(navWidget,SIGNAL(sensorCtrol(QByteArray)),wifithread,SLOT(SendWifiData(QByteArray)));
...
void Widget::receviceMandata(bool flag)
{
    if(flag)
    {
        ui->manLabel->setText(QString::fromLocal8Bit("打开"));
    }else
    {
        ui->manLabel->setText(QString::fromLocal8Bit("关闭"));
    }
}
