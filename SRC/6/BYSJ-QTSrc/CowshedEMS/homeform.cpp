    ...
    ui(new Ui::homeForm)
{
    ...
    tempThresholdValueup = 35;
    tempThresholdValuedown = 25;
...
void homeForm::updataValue(QByteArray sensorData)
{
    ...
    switch((quint8)sensorData[1])
    {
        ...
        case DEVICE_HOT: //加热器
            ...
            if((quint8)sensorData[7] == 0x01)
            {
                QPixmap hotpixmap_on(":/rcs/fancheck_on.png");
                ui->hotpixlabel->setPixmap(hotpixmap_on);
            }
            else
            {
...
void homeForm::runingSmartModel(QByteArray sensorData)
{
    ...
    switch((quint8)sensorData[1])
    {
        ...
        case DEVICE_HT11: //温湿度
            ...
            ui->tempStatelabel->setText(QString::fromLocal8Bit("在线"));
            emit tempcheck(tempData);
            if(tempData > tempThresholdValueup)
            {
                //打开风扇
                SensorCtrolReq(DEVICE_FAN, SETON);
            }else
            ...
            if(tempData < tempThresholdValuedown)
            {
                //打开风扇
                SensorCtrolReq(DEVICE_HOT, SETON);
            }else
...
{
    tempThresholdValueup = ui->tempspinBox->value();
    tempThresholdValuedown = ui->tempspinBox1->value();
}
