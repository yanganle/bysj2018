...
void homeForm::updataValue(QByteArray sensorData)
{
    ...
    switch((quint8)sensorData[1])
    {
        ...
        case DEVICE_WATERLEVEL: //水位
            ...
            break;
        case DEVICE_PH: //PH值
            ui->phNumber->display(sensorStr.setNum(sensorData[7]));
            ui->phStatelabel->setText(QString::fromLocal8Bit("在线"));
            break;
...
void homeForm::SensorSetModelReq(quint8 model)
{
    ...
    cmddata[15] = watlThresholdValue; //温度
    cmddata[16] = model; //模式
    cmddata[17] = 0xFF;
...
void homeForm::resetSensorStatus()
{
    ...
    if(!sensorflag2)
    {
        ui->phStatelabel->setText(QString::fromLocal8Bit("离线"));
    }
    if(!sensorflag3)
    {
