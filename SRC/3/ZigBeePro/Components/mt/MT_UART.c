...
uint8 UART1_RX_STA = 0;
uint8 Uart1RxLen;
uint8 UART1_RX_BUF[127];
...
void MT_UartInit ()
{
  ...
  uartConfig.callBackFunc         = MT_UartProcessZToolData; //串口接受回调函数
 ...
 * @return  None
 ***************************************************************************************************/
void MT_UartProcessZToolData ( uint8 port, uint8 event ) //自定義毀掉函數 1
{
  osal_event_hdr_t  *msg_ptr;
  uint8 uart0RxLen;
  //uint8 dataBuffer0[127];
  
   if(( event & ( HAL_UART_RX_FULL | HAL_UART_RX_ABOUT_FULL | HAL_UART_RX_TIMEOUT ) ) )
   {
     if(port == HAL_UART_PORT_0)
     {
       uart0RxLen = Hal_UART_RxBufLen(HAL_UART_PORT_0);
       
       msg_ptr = (osal_event_hdr_t *)osal_msg_allocate( uart0RxLen + sizeof(osal_event_hdr_t) );
       if(msg_ptr)
       {
         msg_ptr->event = CMD_SERIAL_MSG_0; //串口0
         msg_ptr->status = uart0RxLen;
         HalUARTRead( HAL_UART_PORT_0, (uint8 *)(msg_ptr + 1), uart0RxLen );

         //HalUARTWrite(1, (uint8 *)msg_ptr, 20); 
         osal_msg_send( App_TaskID, (uint8 *)msg_ptr );
       }
     }else //uart1 for sim800a
     { 
       UART1_RX_STA = 1;
       
       Uart1RxLen = Hal_UART_RxBufLen(HAL_UART_PORT_1);
       HalUARTRead(HAL_UART_PORT_1, UART1_RX_BUF, Uart1RxLen);
     }
   }
}

/*void MT_UartProcessZToolData ( uint8 port, uint8 event ) //自定義毀掉函數 2
{
  uint8  ch;
      ...
      case DATA_STATE:
        if(msglen == 18)
        {
          if( ch == 0xFF)
          {
            ...
            {
              if(port == 0)
                pMsg->hdr.event = CMD_SERIAL_MSG_0; //串口0
              else if(port == 1)
                pMsg->hdr.event = CMD_SERIAL_MSG_1; //串口1
              
              osal_memcpy(pMsg->msg, msgBuffer, 18);
...
*/
 ...
 /*
void MT_UartProcessZToolData ( uint8 port, uint8 event )//系统定义回调函数
{
  uint8  ch;
  uint8  bytesInRxBuffer;
