...
#ifdef DEVICE_MQ2 //烟雾传感器
  gasvalue = ReadGasData();
  sdata[0] = 0;
  sdata[1] = 0;
  sdata[2] = gasvalue>>8;//高位
  sdata[3] = gasvalue;//低位
  SampleApp_Send_P2P_Message(DEVICE_MQ2, 0, 0, sdata);
#endif
...
#endif
#ifdef DEVICE_MAN //人体检测
  sdata[0] = 0;
  sdata[1] = 0;
  sdata[2] = 0;
  sdata[3] = GetLightData();
  SampleApp_Send_P2P_Message(DEVICE_MAN, 0, 0, sdata);
#endif 
#ifdef DEVICE_RELAY_PUMP //水泵
  sdata[0] = 0;
  sdata[1] = 0;
  sdata[2] = 0;
  sdata[3] = relay_one_value;
  SampleApp_Send_P2P_Message(DEVICE_RELAY_PUMP, 0, 0, sdata);
#endif    
#ifdef DEVICE_RELAY_LAMP //灯光
  sdata[0] = 0;
  sdata[1] = 0;
  sdata[2] = 0;
  sdata[3] = relay_two_value;
  SampleApp_Send_P2P_Message(DEVICE_RELAY_LAMP, 0, 0, sdata);
#endif    
#ifdef DEVICE_RELAY_COLD //制冷
  sdata[0] = 0;
  sdata[1] = 0;
  sdata[2] = 0;
  sdata[3] = relay_two_value;
  SampleApp_Send_P2P_Message(DEVICE_RELAY_COLD, 0, 0, sdata);
#endif  
...
void SampleApp_NwkCMDMsgProcessing(afIncomingMSGPacket_t * pkt)
{
  ...
  if(buf[0] == 0xEE && buf[1] == 0xCC && buf[17] == 0xFF)
  {
    switch(buf[10])
    {
      ...
      case DEVICE_RELAY_PUMP:
        CtrlRelyOne(buf[16]);
        break;
#endif
#ifdef DEVICE_RELAY_LAMP
      case DEVICE_RELAY_LAMP:
        CtrlRelyTwo(buf[16]);
        break;
#endif
#ifdef DEVICE_RELAY_COLD
      case DEVICE_RELAY_COLD:
        CtrlRelyThree(buf[16]);
        break;
#endif
      default:
        break;
