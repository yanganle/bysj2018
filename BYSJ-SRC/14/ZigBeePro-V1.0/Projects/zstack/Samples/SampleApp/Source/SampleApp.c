...
void SampleApp_Send_CMD_Message(uint8 *pBuf);
/*********************************************************************
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SYS_EVENT_MSG )
  {
	...
	case CMD_SERIAL_MSG_0: 
          ...
          //SampleApp_Send_CMD_Message( ((mtOSALSerialData_t*)MSGpkt)->msg );
          break;
...
void SampleApp_Send_CMD_Message(uint8 *pBuf)
{
  ...
  SampleApp_CMD_DstAddr.addrMode = (afAddrMode_t)afAddr16Bit;
  SampleApp_CMD_DstAddr.endPoint = SAMPLEAPP_ENDPOINT;
  SampleApp_CMD_DstAddr.addr.shortAddr = BUILD_UINT16(pBuf[6], pBuf[5]);

  //HalUARTWrite(1, pBuf, 18); 
  
  if ( AF_DataRequest( &SampleApp_CMD_DstAddr, &SampleApp_epDesc,
                       SAMPLEAPP_CMD_CLUSTERID,
                       0x12,
                       pBuf,
                       &SampleApp_TransID,
...
void SampleApp_NwkP2PMsgProcessing(afIncomingMSGPacket_t * pkt)
{
  ...
  //osal_memcpy(databuffer, sensorValue.dataArry, DATA_AGREEMENT_LENGTH);
  //databuffer[18] = 0x0D;
  //databuffer[19] = 0x0A;
  //HalUARTWrite(0, databuffer,DATA_AGREEMENT_LENGTH+2);
